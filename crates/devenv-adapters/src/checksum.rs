...
use devenv_core::{ChecksumVerifier, CoreError, CoreResult};

#[derive(Debug, Clone, Default)]
pub struct Sha256ChecksumVerifier;

impl ChecksumVerifier for Sha256ChecksumVerifier {
    fn verify(&self, artifact_path: &Path, expected_checksum: &str) -> CoreResult<()> {
                ...
                .all(|character| character.is_ascii_hexdigit())
        {
            return Err(CoreError::message(format!(
                "unsupported checksum `{expected_checksum}`: expected sha256:<hex> or raw sha256 hex"
            )));
        }
